class Game
  class Character
    ...
    def initialize
      ...
      print "Enter the gender of the character (male/female/non-binary):"
      @gender = gets.chomp.downcase

      print "Enter the age of the character:"
      @age = gets.chomp.to_i
  ...
  class Barbarian < Character
    ...
    def choose_poison
      loop do
        puts "Choose what poison to choose"
        puts "1. Light Health poison"
        puts "2. Middle Health poison"
        puts "3. Strong Health poison"
        puts "4. Mana poison"
        poison = gets.chomp.to_i
        ...
        when 3
          ...
          break
        when 4
          puts "You don't have Mana poison"
          break
        else
          puts "Unknown poison, choose again"
        end
      end
    end
  end
  ...
  def self.battle(character, enemy)
    ...
    while character.hp > 0 && enemy.hp > 0
      ...
      if (character.is_a?(Mage) || character.is_a?(Druid)) && character.mana > 0
        character.attack(enemy)
        character.mana -= 10
        puts "#{enemy.name} has #{enemy.hp} HP left."
        puts "You have #{character.mana} points of mana"
      elsif character.is_a?(Barbarian)
        character.attack(enemy)
        puts "#{enemy.name} has #{enemy.hp} HP left."
      end
      ...
      if enemy.hp > 0
        ...
        #ИСПОЛЬЗУЕМ ЗЕЛЬЕ, ЕСЛИ ЗДОРОВЬЕ БОЛЬШЕ НУЛЯ
        if character.hp > 0
          puts "Do you want to use a health or mana poison? (y/n)"
          choice = gets.chomp.downcase
          if choice == 'y'
            character.choose_poison
            puts "Now you have #{character.hp} and #{character.hp}"
          end
        end
      end
      puts "Press Enter to continue..."
...
#ДОБАВИТЬ ЛИМИТ НА ВОССТАНОВЛЕНИЕ ЗДОРОВЬЯ ПЕРСНОАЖА